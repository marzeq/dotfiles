...
class BluetoothWidget(ControlCentreWidget):
    def __init__(self):
        ...
        super().__init__(
            icon=bluetooth.bind(
                "state",
                lambda state: "bluetooth-active-symbolic"
                if state == "on" and bluetooth.powered
                else "bluetooth-disabled-symbolic",
            ),
            labels=CCWLabels("Bluetooth"),
            on_click=lambda _: bluetooth.set_powered(False)
            if bluetooth.powered
            else bluetooth.set_powered(True),
        ...
        bluetooth.connect(
            "notify::state",
            lambda *_: self.set_disabled(
                bluetooth.state == "absent" or not bluetooth.powered
            ),
        )
        bluetooth.connect(
            "notify::powered",
