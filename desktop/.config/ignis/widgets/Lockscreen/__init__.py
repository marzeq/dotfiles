...
class EntryScreen(Widget.Box):
    def __init__(self, on_accept: Callable[[str], Any], on_change: Callable[[], Any]):
        ...
        self.entry.set_icon_from_icon_name(Gtk.EntryIconPosition.SECONDARY, "view-conceal-symbolic")
        ...
        pfp_frame.set_child(Widget.Icon(
            hexpand=False,
            valign="center",
            halign="center",
        ...
        super().__init__(
            ...
            child=[
                ...
                Widget.Label(
                    ...
                    css_classes=["lockscreen-username"]
                ),
                self.entry
            ]
        )
...
class LockScreen(Widget.Window):
    def __init__(self, lock_instance, monitor: Gdk.Display, monitor_id: int):
        super().__init__(
            visible=False,
            layer="top",
            namespace=f"ignis_lockscreen_{monitor_id}"
        )
        ...
        entry_screen = EntryScreen(
            on_accept=lambda t: None if self.authenticating else asyncio.create_task(self._on_accept(t)),
        ...
        key_controller.connect("key-pressed", lambda *x: self._handle_keypress(x[1] == 65307, x[1]))
